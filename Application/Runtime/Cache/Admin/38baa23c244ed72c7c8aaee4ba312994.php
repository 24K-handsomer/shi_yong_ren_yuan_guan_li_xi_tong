...
<body>
	...
	<div id="row">
	<form name='MyForm' id='MyForm' method='GET' action="/index.php/Admin/Test/index" >
        <span>文件类型：</span>
        <select name="fileid">
        ...
        <span>和</span>
        <select name="typeid">
            <?php if(is_array($questiontype)): foreach($questiontype as $key=>$vv): ?><option value ="<?php echo ($vv["typeid"]); ?>"><?php echo ($vv["tyname"]); ?></option><?php endforeach; endif; ?>
        </select>
	...
	<table border="1" cellpadding="0" cellspacing="0">
		...
		<?php if(is_array($list)): foreach($list as $key=>$v3): ?><tr id="">
			<td><?php echo $i++; ?></td>
			<td>
				<?php if(is_array($filetype)): foreach($filetype as $key=>$v4): if($v3["filetype"] == $v4['fileid']): echo ($v4["filename"]); endif; endforeach; endif; ?>
			</td>
			<td>
				<?php if(is_array($questiontype)): foreach($questiontype as $key=>$v5): if($v3["type"] == $v5['typeid']): echo ($v5["tyname"]); endif; endforeach; endif; ?>
			</td>
			<td>
				<?php echo ($v3["question"]); ?>
			</td>
			<td><?php echo ($v3["crtime"]); ?></td>
			<td>
				<a href="/index.php/Admin/Test/revisetest?id=<?php echo ($v3["id"]); ?>">查看</a>&nbsp;&nbsp;|&nbsp;&nbsp;
				<a href="/index.php/Admin/Test/revisetest?id=<?php echo ($v3["id"]); ?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
				<?php if($v3["status"] == 1): ?><input type="submit" class="status" name="status" xu="<?php echo ++$j ?>" ps="<?php echo ($v3["id"]); ?>" status="<?php echo ($v3["status"]); ?>" value="禁用">
				<?php else: ?>
				<input type="submit" class="status" name="status" xu="<?php echo ++$j ?>" ps="<?php echo ($v3["id"]); ?>" status="<?php echo ($v3["status"]); ?>" value="启用"><?php endif; ?>
			</td>
...
<script type="text/javascript">
 $(function(){
 	$("input[name='status']").click(function(){
 		...
 		$.ajax({
 			url:"/index.php/Admin/Test/status",
