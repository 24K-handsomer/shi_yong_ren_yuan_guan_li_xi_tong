...
<body>
	...
	<div id="row">编辑试题</div>
	<div id="table-top">
	<form action="/index.php/Admin/Test/update" method="post">
		<input type="hidden" name="id" value="<?php echo ($question["id"]); ?>">
		<span>考试类别1:</span>
		<select name="fileid" id="fileid">
            <?php if(is_array($filetype)): foreach($filetype as $key=>$v): if($v["fileid"] == $question['filetype']): ?><option selected="selected" value ="<?php echo ($v["fileid"]); ?>"><?php echo ($v["filename"]); ?></option>
		        <?php else: ?>
		        	<option value ="<?php echo ($v["fileid"]); ?>"><?php echo ($v["filename"]); ?></option><?php endif; endforeach; endif; ?>
        </select>
        <span>考试类别2:</span>
        <select name="typeid" id="typeid">
            <?php if(is_array($questiontype)): foreach($questiontype as $key=>$vv): if($vv["typeid"] == $question['type']): ?><option selected="selected" value ="<?php echo ($vv["typeid"]); ?>"><?php echo ($vv["tyname"]); ?></option>
            	<?php else: ?>
            		<option value ="<?php echo ($vv["typeid"]); ?>"><?php echo ($vv["tyname"]); ?></option><?php endif; endforeach; endif; ?>
        </select>
        <table border="1" cellpadding="0" cellspacing="0">
        	<tr>
        		<td>题目内容</td>
        		<td>
        			<textarea name="content"><?php echo ($question["question"]); ?></textarea>
        		</td>
        	</tr>
...
$(function(){
	...
	//判断问题类型
	function xuanxiang(){
		var typeid = <?php echo ($question["type"]); ?>;
		switch (typeid) {
	...
	function selectnumber(){
		...
		var shumu = <?php echo ($question["selectnumber"]); ?>;
		for (var i=4;i>=1;i--) {
			if (i == shumu) {
	...
	function showselect_1() {
		var shumu = <?php echo ($question["selectnumber"]); ?>;
		var str1 = "";
		var select = "<?php echo ($question["select"]); ?>";
	...
	//判断选择题的答案
	function answer(){
		var answer = "<?php echo ($question["answer"]); ?>";
